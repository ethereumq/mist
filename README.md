...
*NOTE: client-binaries (e.g. [gethq](https://github.com/ethereumq/go-ethereumq)) specified in [clientBinaries.json](https://github.com/ethereumq/minst/blob/master/clientBinaries.json) will be checked during every startup and downloaded if out-of-date, binaries are stored in the [config folder](#config-folder)*
...
### Passing options to Gethq

You can pass command-line options directly to Gethq by prefixing them with `--node-` in
...
```bash
$ yarn dev:electron --rpc /my/gethq.ipc
```
...
```bash
$ yarn dev:electron --rpc /my/geth.ipc --node-ipcpath /my/gethq.ipc
```
...
```bash
$ yarn dev:electron --rpc ~/Library/EthereumQuantum/gethq.ipc --node-networkid 1234 --node-datadir ~/Library/EthereumQuantum/privatenet
```

_NOTE: since `ipcpath` is also a Mist option you do not need to also include a
`--node-ipcpath` option._

You can also launch `gethq` separately with the same options prior starting
Mist.
