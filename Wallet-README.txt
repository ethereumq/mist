...
On OSX its `~/Library/EthereumQuantum/gethq.ipc` on linux `~/ethereumquantum/gethq.ipc` and on windows it uses a named pipe, which doesn't need to be renamed.

Example:

    $ gethq --datadir /my/chain/ --networkid 23 --ipcpath ~/Library/EthereumQuantum/gethq.ipc
...
- Windows: %APPDATA%\Roaming\EthereumQuantum
- Linux: ~/.ethereumquantum
...
https://github.com/ethereumq/minst/tree/wallet#deployment
